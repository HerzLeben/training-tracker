  ...
  if (!plan.hasTargets) {
    return (
      <div className={`${CARD} p-4 text-sm text-slate-500`}>
        Set target body fat / muscle and a date in <span className="text-slate-700">Settings</span> to
        see your plan here.
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between">
        ...
        {plan.targetDate ? (
          <div className="text-xs text-slate-500">
            by {plan.targetDate}
            {plan.weeksLeft !== null && (
              <span className="ml-1 text-slate-400">({Math.ceil(plan.weeksLeft)} wk left)</span>
            )}
          </div>
        ) : (
          <div className="text-xs text-slate-400">no target date set</div>
        )}
      </div>
