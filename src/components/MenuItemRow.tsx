  ...
  return (
    <div className={`rounded-xl border p-3 transition ${itemBorder(item.done)}`}>
      ...
      <div className="mt-2 flex items-center gap-2">
        <div className="flex flex-1 items-center justify-between rounded-lg bg-slate-50 px-2 py-1.5">
          <span className="text-xs text-slate-500">Weight</span>
          <div className="flex items-center gap-2">
            <button
              onClick={() => adjust(-STEP)}
              className="h-7 w-7 rounded-full bg-slate-200 text-lg leading-none text-slate-700 active:bg-slate-300"
              aria-label="decrease weight"
            >
              −
            </button>
            <span className="min-w-[3.5rem] text-center text-base font-semibold tabular-nums text-slate-800">
              {w === undefined ? '—' : w}
        ...
        <div className="flex items-center gap-1 rounded-lg bg-slate-50 px-2 py-1.5">
          <input
            ...
            placeholder="—"
            className="w-12 bg-transparent text-center text-base font-semibold tabular-nums text-slate-800 outline-none"
